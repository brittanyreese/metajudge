...
elsewhere in this repo; here it is a number, not a warning in prose. Its
threshold was calibrated on one data-generating family and two confound
mechanisms (docs/decisions/2026-07-02-e07-overlap-threshold-calibration.md),
so read a firing flag as a strong prompt to check the conditioner, not a
certified detector.
...
def render(cards: dict[str, dict[str, ReportCard]]) -> str:
    ...
    lines = [
        ...
        f"flags the corrupted conditioner automatically, before anyone has to "
        f"notice the gap by hand.",
        "",
