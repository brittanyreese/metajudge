...
- **Ollama.** The default context window is 4096 tokens (`OLLAMA_CONTEXT_LENGTH` env var, or `options.num_ctx` per-request), independent of `temperature`/`seed`. A long essay plus the per-trait prompt overhead can approach that ceiling and silently truncate the input rather than error. Set `options.num_ctx` explicitly to comfortably exceed the longest essay plus prompt overhead, and treat the response's `prompt_eval_count` as a truncation tripwire (compare it against the tokenized length of what was actually sent).

None of this is implemented in `_ellipse_judge.py` yet (`JudgeConfig` has no `provider`/`num_ctx` fields); it is the plan for the next change, ahead of any paid re-trial.
