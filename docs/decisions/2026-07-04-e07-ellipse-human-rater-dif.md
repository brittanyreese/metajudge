...
Since the isolated per-trait design rules out cross-call leakage architecturally, and the collapsed scores reproduce identically across independently issued requests, this is not the batching-anchoring bug recurring. It reads as GPT-4o genuinely converging on the same holistic quality read for these particular essays (two of the three diagnostic essays are short: 1336 and 789 characters), not as evidence the fix failed to isolate the calls. **This is a plausible reading from 3 essays, not a proven one.** An equally live alternative is that the keyword-rubric prompt does not give GPT-4o enough trait-specific discriminating signal on short or clearly low/high-proficiency essays, and it falls back to one overall impression per call rather than genuinely reasoning about the specific trait asked, isolated or not. Both readings point to the same practical conclusion (no batching leakage, so the isolation fix did what it was supposed to do), but they diverge on whether GPT-4o is a good analytic-trait judge for this rubric design on essays like these, which is a separate, larger question this 3-essay check cannot settle.
...
**Before any full-corpus GPT-4o run:** the collapse rate needs to be understood well enough to decide whether the pilot's DIF numbers on Vocabulary/Syntax/Overall would still be a clean measurement at this collapse rate, or whether a rubric redesign (chain-of-thought before the score, or per-trait few-shot anchors) is needed first. That decision is out of scope for this pins change and is the next open question, not the 6,462-essay spend itself.
