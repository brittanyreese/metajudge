...
- The rest score is contaminated when the bias is shared across the rater panel. It removes the studied rater's own response but not a bias every rater carries, so for instrument-level bias (the same judge applied to a group) it understates the DIF. On the SummEval demo this shows up directly: the frozen fixture, built with a uniform bias shared across raters, lands at class C under an independent conditioner but class A under the rest score. Both numbers are pinned in the tests as an honest illustration, and the rule is stated in the function docstring: use an independent external conditioner for instrument-level bias; the rest score detects panel-relative (differential-rater) functioning.
