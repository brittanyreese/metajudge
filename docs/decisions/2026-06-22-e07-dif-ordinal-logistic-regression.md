...
- With stratum assigned per unit, the group and the conditioner vary at the unit level while the response varies by rater, so rows within a unit are not independent. The standard errors are mildly optimistic, so the card frames the result as a screening audit, not a confirmatory significance claim.
- The proportional-odds assumption is not tested here. Where raters use the scale differently it can be violated; lordif mitigates this with an IRT-theta conditioner, which this scope does not implement.
